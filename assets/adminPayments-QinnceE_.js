import{c as l,p as d,d as c,f as m,s as r,g as u}from"./ui-wgqUTf1L.js";/* empty css              *//* empty css                  */import{c as g,r as f,a as p}from"./payment-BhoriJkW.js";const y=await l(["admin"]);d(y);c();document.getElementById("logout-btn").addEventListener("click",m);let i=[],s="pending";async function h(n,e=[]){try{return await n}catch(t){return console.error("Fetch error:",t),e}}async function a(){i=await h(p(),[]);const n=i.filter(t=>t.status==="pending").length,e=document.getElementById("cnt-pending");e&&(e.textContent=n),o()}function o(){const n=document.getElementById("payments-container");let e=i;if(s!=="all"&&(e=i.filter(t=>t.status===s)),!e.length){n.innerHTML=`<div style="text-align:center;padding:3rem;background:#fff;border-radius:var(--radius-xl);border:1px dashed #cbd5e1">
  ...
  `).join("")}document.getElementById("filter-tabs").addEventListener("click",n=>{const e=n.target.closest(".filter-tab");e&&(document.querySelectorAll(".filter-tab").forEach(t=>t.classList.remove("active")),e.classList.add("active"),s=e.dataset.status,o())});window.handleApprove=async(n,e)=>{if(confirm("මෙම ගෙවීම අනුමත කිරීමට විශ්වාසද?"))try{await g(n,e),r("ගෙවීම අනුමත කරන ලදී ✅","success"),a()}catch(t){r(t.message,"error")}};window.handleReject=async(n,e)=>{if(confirm("මෙම ගෙවීම ප්‍රතික්ෂේප කිරීමට විශ්වාසද?"))try{await f(n,e),r("ගෙවීම ප්‍රතික්ෂේප කරන ලදී","warning"),a()}catch(t){r(t.message,"error")}};a();
