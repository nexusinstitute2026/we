import{r as $,g as E,s as a,c as L}from"./auth-qdlMy9e7.js";/* empty css              *//* empty css             *//* empty css               */import{a as M}from"./quiz-BxfecA8q.js";import{s as c}from"./ui-CfSCUwhT.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";import"./leaderboard-P9WSWk3A.js";await $(["student","teacher","admin"]);const n=await E();n||(window.location.href="../login.html");const b=new URLSearchParams(window.location.search),l=b.get("id"),p=b.get("month");window.switchTab=s=>{document.querySelectorAll(".tab-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".tab-content").forEach(t=>t.classList.remove("active")),event.currentTarget.classList.add("active"),document.getElementById(`tab-${s}`).classList.add("active")};async function S(){if(!l)return;const{data:s}=await a.from("courses").select("name, is_free, subject:subjects(name, teacher:profiles(full_name))").eq("id",l).single();s&&(document.getElementById("c-title").textContent=`${s.subject?.name} - ${s.name}`,document.getElementById("c-teacher").querySelector("span").textContent=s.subject?.teacher?.full_name||"Teacher");let t=a.from("sessions").select("*").eq("course_id",l);if(p){const{data:e}=await a.from("course_months").select("year, month_number, name, is_free").eq("id",p).single();if(e){if(document.getElementById("c-title").textContent+=` - ${e.year} ${e.name}`,n.role==="student"){const r=await L(n.id,l),v=e.year*12+(e.month_number-1)<=r.maxAccessibleValue,I=r.paidMonths.some(y=>y.year===e.year&&y.month===e.month_number);if(!(s?.is_free||e.is_free||r.hasCurrentAccess&&v||I)){document.querySelector("main").innerHTML=`
    ...
    `).join("");const{data:d}=await a.from("assignments").select("*").eq("course_id",l).order("due_date",{ascending:!0}),_=document.getElementById("assign-list");d&&d.length>0&&(_.innerHTML=d.map(e=>{const r=e.description?.match(/\[MAX_MB:(\d+)\]/),u=r?parseInt(r[1]):10,v=e.description?.replace(/\[MAX_MB:\d+\]/,"").trim()||"";return`
      ...
      `}).join(""));const x=await M(l),A=new Date,f=x.filter(e=>{const r=e.status==="published",u=!e.publish_at||new Date(e.publish_at)<=A;return r&&u}),B=document.getElementById("quiz-list");f&&f.length>0&&(B.innerHTML=f.map(e=>`
      <div class="session-card">
        <div class="session-header">
