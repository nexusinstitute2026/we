import{r as h,p as f,l as v}from"./auth-5l32Gasm.js";/* empty css              *//* empty css             *//* empty css                  */import{a as b,r as w,g as x}from"./payment-RUGV7WNt.js";import{v as _,w as $,z as k}from"./admin-BXWvPR6F.js";import{i as E,s,a as B}from"./ui-CfSCUwhT.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";import"./preload-helper-BKjL65nl.js";const A=await h(["admin"]);f(A);E();document.getElementById("logout-btn").addEventListener("click",v);let l=[],o="pending",c="",m="";async function L(n,a=[]){try{return await n}catch(e){return console.error("Fetch error:",e),a}}async function u(){l=await L(x(),[]);const n=l.filter(e=>e.status==="pending").length,a=document.getElementById("cnt-pending");a&&(a.textContent=n),g()}function g(){const n=document.getElementById("payments-container"),a={1:"ජනවාරි",2:"පෙබරවාරි",3:"මාර්තු",4:"අප්‍රේල්",5:"මැයි",6:"ජූනි",7:"ජූලි",8:"අගෝස්තු",9:"සැප්තැම්බර්",10:"ඔක්තෝබර්",11:"නොවැම්බර්",12:"දෙසැම්බර්"};if(o==="manual"){const t=l.filter(r=>r.slip_drive_link==="manual_assign");let i="";t.length?i=t.map(r=>{const d=r.payment_courses?.map(y=>y.course?.name).filter(Boolean).join(", ")||"—",p=r.paid_month?`${r.paid_year||""} ${a[r.paid_month]||r.paid_month}`:"—";return`
        ...
        </div>`}).join(""):i='<div style="text-align:center;padding:2rem;color:var(--text-muted)">අතින් ලබා දුන් ප්‍රවේශයන් නොමැත.</div>',n.innerHTML=`
    ...
    </div>`;return}n.innerHTML=e.map(t=>{const i=t.payment_courses?.map(d=>d.course?.name).filter(Boolean).join(", ")||"—",r=t.paid_month?`${t.paid_year||""} ${a[t.paid_month]||t.paid_month}`:"—";return`
  ...
  `}).join("")}document.getElementById("filter-tabs").addEventListener("click",async n=>{const a=n.target.closest(".filter-tab");a&&(document.querySelectorAll(".filter-tab").forEach(e=>e.classList.remove("active")),a.classList.add("active"),o=a.dataset.status,o==="manual"&&(document.getElementById("payments-container").innerHTML='<div style="text-align:center;padding:2rem">Loading...</div>',await j()),g())});async function j(){if(!c)try{const[n,a]=await Promise.all([_("student"),$()]);c='<option value="">— තෝරන්න —</option>'+n.map(e=>`<option value="${e.id}">${e.full_name} (${e.whatsapp_number})</option>`).join(""),m='<option value="">— තෝරන්න —</option>',a.forEach(e=>{e.categories.forEach(t=>{t.subjects.forEach(i=>{i.courses.forEach(r=>{m+=`<option value="${r.id}">${e.name} - ${i.name} - ${r.name}</option>`})})})})}catch(n){s(n.message,"error")}}window.submitManualAssign=async()=>{const n=document.getElementById("manual-student-select").value,a=document.getElementById("manual-course-select").value;if(!n||!a)return s("ශිෂ්‍යයෙක් සහ පාඨමාලාවක් තෝරන්න","warning");try{await k(n,a),s("පාඨමාලාව සාර්ථකව ලබා දෙන ලදී ✅","success"),document.getElementById("manual-course-select").value=""}catch(e){s(e.message,"error")}};window.handleApprove=async(n,a)=>{if(confirm("මෙම ගෙවීම අනුමත කිරීමට විශ්වාසද?"))try{await b(n,a),s("ගෙවීම අනුමත කරන ලදී ✅","success"),u()}catch(e){s(e.message,"error")}};window.handleReject=async(n,a)=>{if(confirm("මෙම ගෙවීම ප්‍රතික්ෂේප කිරීමට විශ්වාසද?"))try{await w(n,a),s("ගෙවීම ප්‍රතික්ෂේප කරන ලදී","warning"),u()}catch(e){s(e.message,"error")}};u();
