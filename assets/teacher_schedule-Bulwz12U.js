import{r as d,p as u,l as m}from"./auth-5l32Gasm.js";/* empty css              *//* empty css             *//* empty css                  */import{K as p}from"./admin-BPFFsS_i.js";import{i as g,s as o}from"./ui-CfSCUwhT.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";import"./preload-helper-BKjL65nl.js";const r=await d(["teacher","admin"]);u(r);g();document.getElementById("logout-btn").addEventListener("click",m);let i=null;async function y(){const n=await p(r.id),s=document.getElementById("course-select"),a=[];n.forEach(t=>t.courses?.forEach(e=>a.push({id:e.id,name:`${t.name} — ${e.name}`}))),s.innerHTML='<option value="">-- පාඨමාලාව තෝරන්න --</option>'+a.map(t=>`<option value="${t.id}">${t.name}</option>`).join(""),s.addEventListener("change",()=>{i=s.value,i&&l()})}async function l(){const n=document.getElementById("classes-container");n.innerHTML='<div class="skeleton" style="height:100px;border-radius:1rem;margin-bottom:1rem"></div>';try{const a=await(await fetch(getApiUrl(`/api/classes?teacherId=${r.id}&courseId=${i}`))).json();if(!a.success)throw new Error(a.message);const t=a.data;if(!t.length){n.innerHTML='<div style="text-align:center;padding:4rem;color:var(--text-muted)"><h3>පන්ති නොමැත</h3></div>';return}n.innerHTML=t.map(e=>`
