import{c as $,h as E,s as c,b as a,j as L}from"./ui-BjnYUZX8.js";/* empty css              *//* empty css               */import{a as M}from"./quiz-0TBxxybf.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";await $(["student","teacher","admin"]);const n=await E();n||(window.location.href="../login.html");const b=new URLSearchParams(window.location.search),l=b.get("id"),p=b.get("month");window.switchTab=s=>{document.querySelectorAll(".tab-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".tab-content").forEach(t=>t.classList.remove("active")),event.currentTarget.classList.add("active"),document.getElementById(`tab-${s}`).classList.add("active")};async function S(){if(!l)return;const{data:s}=await a.from("courses").select("name, subject:subjects(name, teacher:profiles(full_name))").eq("id",l).single();s&&(document.getElementById("c-title").textContent=`${s.subject?.name} - ${s.name}`,document.getElementById("c-teacher").querySelector("span").textContent=s.subject?.teacher?.full_name||"Teacher");let t=a.from("sessions").select("*").eq("course_id",l);if(p){const{data:e}=await a.from("course_months").select("year, month_number, name, is_free").eq("id",p).single();if(e){if(document.getElementById("c-title").textContent+=` - ${e.year} ${e.name}`,n.role==="student"){const d=await L(n.id,l),f=e.year*12+(e.month_number-1)<=d.maxAccessibleValue,I=d.paidMonths.some(y=>y.year===e.year&&y.month===e.month_number);if(!(e.is_free||d.hasCurrentAccess&&f||I)){document.querySelector("main").innerHTML=`
          ...
          `;return}}t=t.eq("course_month_id",p)}}const{data:o}=await t.order("date",{ascending:!1});let i=o||[];const g=document.getElementById("sessions-list");!i||i.length===0?g.innerHTML='<div style="text-align:center;padding:3rem;background:#fff;border-radius:1rem;border:1px dashed #cbd5e1"><h3 style="color:var(--text-muted)">පන්ති දින සටහන් තවම යාවත්කාලීන වී නොමැත.</h3></div>':g.innerHTML=i.map(e=>`
