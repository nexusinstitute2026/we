import{r as u,p as g,l as p}from"./auth-qdlMy9e7.js";/* empty css              *//* empty css             *//* empty css                  */import{a as h,r as y,g as f}from"./payment-rGI2tEmi.js";import{i as v,s as i,a as b}from"./ui-CfSCUwhT.js";import"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";const w=await u(["admin"]);g(w);v();document.getElementById("logout-btn").addEventListener("click",p);let s=[],a="pending";async function x(n,e=[]){try{return await n}catch(r){return console.error("Fetch error:",r),e}}async function o(){s=await x(f(),[]);const n=s.filter(r=>r.status==="pending").length,e=document.getElementById("cnt-pending");e&&(e.textContent=n),l()}function l(){const n=document.getElementById("payments-container");let e=s;if(a!=="all"&&(e=s.filter(t=>t.status===a)),!e.length){n.innerHTML=`<div style="text-align:center;padding:3rem;background:#fff;border-radius:var(--radius-xl);border:1px dashed #cbd5e1">
  ...
  `}).join("")}document.getElementById("filter-tabs").addEventListener("click",n=>{const e=n.target.closest(".filter-tab");e&&(document.querySelectorAll(".filter-tab").forEach(r=>r.classList.remove("active")),e.classList.add("active"),a=e.dataset.status,l())});window.handleApprove=async(n,e)=>{if(confirm("මෙම ගෙවීම අනුමත කිරීමට විශ්වාසද?"))try{await h(n,e),i("ගෙවීම අනුමත කරන ලදී ✅","success"),o()}catch(r){i(r.message,"error")}};window.handleReject=async(n,e)=>{if(confirm("මෙම ගෙවීම ප්‍රතික්ෂේප කිරීමට විශ්වාසද?"))try{await y(n,e),i("ගෙවීම ප්‍රතික්ෂේප කරන ලදී","warning"),o()}catch(r){i(r.message,"error")}};o();
